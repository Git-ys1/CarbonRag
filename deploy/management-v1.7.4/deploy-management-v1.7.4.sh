...
SKIP_BACKEND_DEPENDENCIES="${SKIP_BACKEND_DEPENDENCIES:-false}"
...
if [ "${SKIP_BACKEND_DEPENDENCIES}" = "true" ]; then
  echo "[V1.7.4] skipped backend dependency install"
else
...
if [ "${SKIP_FRONTEND_BUILD}" = "true" ]; then
  echo "[V1.7.4] skipped frontend build"
else
